...
# Constants
COLOR_PRIMARY = lv.palette_main(lv.PALETTE.BLUE_GREY)
...
if style_debug:
    style_debug.init()
    if DEBUG:
        style_debug.set_border_color(lv.color_make(255, 0, 255))
...
if style_screen:
    ...
    style_screen.set_pad_left(GLOBAL_PADDING)
    style_screen.set_pad_right(GLOBAL_PADDING)
    style_screen.set_pad_top(GLOBAL_PADDING)
    style_screen.set_pad_bottom(GLOBAL_PADDING)
...
if style_button:
    style_button.init()
    style_button.set_bg_color(lv.color_lighten(COLOR_BACKGROUND, 16))
    style_button.set_border_color(lv.color_lighten(COLOR_BACKGROUND, 48))
...
if style_button_icon:
    ...
    style_button_icon.set_bg_color(lv.color_lighten(COLOR_BACKGROUND, 16))
    style_button_icon.set_border_color(lv.color_lighten(COLOR_BACKGROUND, 48))
    style_button_icon.set_border_width(1)
    style_button_icon.set_border_side(lv.BORDER_SIDE.FULL)
    style_button_icon.set_shadow_width(0)
    style_button_icon.set_width(lv.dpx(60))
    style_button_icon.set_height(lv.dpx(60))
...
style_flex_container = lv.style()
if style_flex_container:
    style_flex_container.init()
    style_flex_container.set_bg_color(lv.color_lighten(COLOR_BACKGROUND, 96))
    style_flex_container.set_bg_opa(lv.OPA_TRANSP)
    style_flex_container.set_border_width(0)
    style_flex_container.set_radius(0)

# def theme_apply_cb(theme, object):
#     test = object.get_class()
#     pass

# theme = lv.theme_default_get()
# theme = lv.theme_default_init(None, COLOR_PRIMARY, COLOR_BACKGROUND, True, font_text)
# theme.set_apply_cb(theme_apply_cb)
...
def flex_container(parent, flow = lv.FLEX_FLOW.COLUMN, align = lv.FLEX_ALIGN.START):
    ...
    result.add_style(style_flex_container, lv.STATE_DEFAULT)
    result.add_style(style_debug, lv.STATE_DEFAULT)
